...
| Training observability | [V6 failure audit](evaluation/v6-validation/README.md), [machine-readable findings](evaluation/v6-validation/report/failure_analysis.json) | A completed 3,823-step run records decoded checkpoint drift, prompt-intent slices, reasoning traces, retries and signal ablations | This is a repeatedly inspected 84-row validation diagnostic, not a new test benchmark |
...
| Working evidence-linked demo | [Demo script](DEMO.md), [smoke receipt](demo/smoke-result.json), [readable handoff](demo/investigation-example.md), [audit JSON](demo/investigation-example.json) | Private canary-v4 path was smoke-tested; exports retain telemetry receipt, measurements and prediction separately | The three fixed examples are not a quality benchmark; raw coverage remains limited |
...
The archived confusion matrices localize OpenTSLM's gap. Of 512 windows, 113 received no usable structured answer (112 of 241 free-motion windows, 1 of 271 contact windows). On the 399 answered windows, OpenTSLM's semantics accuracy is **98.50%** (393/399), against the baseline's 98.83% overall. The dominant failure mode is therefore missing structured output on free-motion windows (46.5% abstention on that class), not wrong event interpretation when an answer is produced. This is a post-hoc decomposition of already-inspected test results, not a new benchmark: conditioning on "answered" selects easier windows for the model, so the two accuracies are not a paired comparison, and any fix for the abstention behavior must be selected on validation.
