...
|---|---|---|
| 0:00–0:18 | Start on the selected publisher event with the pre-run result ready | “After a recorded contact event, an engineer inspects seven traces and writes the incident handoff. Trace turns that event into a reviewable hypothesis, measured cross-check and exact evidence. This compact fixture comes from open KUKA data.” |
...
| 1:43–2:00 | Open **Evaluation** and close on the headline comparison | “Across 512 held-out windows, simple features win classification and OpenTSLM is usable 77.93 percent of the time. Trace’s contribution is the reviewable, reproducible handoff; a timed pilot must still prove that workflow value.” |
