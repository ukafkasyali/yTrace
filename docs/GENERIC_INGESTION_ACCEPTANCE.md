...
| Generic TimeF | Wide CSV, long CSV, named NPZ | `test_generic_import.py`; exact record, channel, unit, value and irregular-time read-back |
...
`test_mapping.py` records the pause/resume boundary: ambiguous selectors and unknown units cannot
advance, while a confirmed mapping is revision-bound and immutable. `test_generic_import.py`
