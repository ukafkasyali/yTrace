...
class RawRecording:
    ...
    def raw_window(self, start_seconds: float, end_seconds: float, channel_ids: list[str]):
        ...
        lo = int(np.searchsorted(times, start_seconds, side="left"))
        hi = int(np.searchsorted(times, end_seconds, side="left"))
        selected_times = times[lo:hi]
        if not len(selected_times):
            raise ValueError("Raw source has no samples in this interval")
        if selected_times[0] != start_seconds or selected_times[-1] >= end_seconds:
            raise ValueError("Raw source does not contain the requested half-open interval")
    ...
    def replay_data(self):
        ...
        detail_times, detail_values = self.raw_window(0.0, min(1.0, self.duration_seconds), list(CHANNEL_IDS))
        ...
        return {
            ...
            "detail": {"startSeconds": 0.0, "endSeconds": min(1.0, self.duration_seconds),
