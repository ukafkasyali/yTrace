...
export default function ScoutReview({ run, busy, onReview, onUseSource }: Props) {
  ...
  return <div className="scout-review" aria-live="polite">
    <div className="scout-run-line"><span className={`run-status status-${run.status.toLowerCase()}`}>{run.status.replaceAll('_', ' ')}</span><span>mode: {run.executionMode.toLowerCase()} · {run.tavilyCreditsUsed}/12 Tavily credits · {run.familyQueriesUsed}/1 family search · {run.gapQueriesUsed}/2 gap searches</span><span className="mono">{run.runId}</span></div>
