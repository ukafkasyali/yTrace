...
export default function RobotPanel({ prediction, data, playhead, interval, highlighted, onHighlight }: Props) {
  ...
  return <div className="robot-panel">
    ...
    <div className={`robot-event-cue${atOnset ? ' at-onset' : ''}`} aria-label="Robot event context">
      {prediction ? <><strong>{atOnset ? 'Predicted onset' : playhead < prediction.onsetSeconds ? 'Before onset' : 'After onset'} · {prediction.onsetSeconds.toFixed(3)} s</strong><span>{prediction.title.replace(' predicted', '')}{prediction.channelId ? ` · J${prediction.channelId.slice(-1)}` : ''} · time cue only</span></> : selectedMarker ? <><strong>Publisher marker · {selectedMarker.timeSeconds.toFixed(3)} s</strong><span>{rawAvailable ? 'Annotation · analyze for a model prediction' : 'Annotation · overview measurements only'}</span></> : <><strong>Recorded movement</strong><span>Select an event to investigate its torque.</span></>}
    </div>
