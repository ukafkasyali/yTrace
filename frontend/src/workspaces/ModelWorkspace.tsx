...
import { DECLARED_MODELS, type Evidence, type ModelProfile, type QueryRequest, type Services } from '../services';

type Props = { services: Services; request: QueryRequest; onEvidence: (evidence: Evidence) => void };
type Result = {
...
export default function ModelWorkspace({ services, request, onEvidence }: Props) {
  const [models, setModels] = useState<ModelProfile[]>(DECLARED_MODELS.map(model => ({ ...model })));
  ...
  useEffect(() => {
    let alive = true;
    services.listModels().then(items => {
      if (!alive) return;
      setModels(DECLARED_MODELS.map(model => items.find(item => item.id === model.id) ?? { ...model, reason: 'Not returned by model service' }));
    }).catch(reason => { if (alive) { setError(errorText(reason)); setModels(DECLARED_MODELS.map(model => ({ ...model }))); } });
    return () => { alive = false; };
  }, [services]);
  ...
  async function compare() {
    ...
    if (!services.connected || !available.length) return;
    const snapshot = structuredClone(request); const run = ++generation.current;
    ...
    await Promise.allSettled(available.map(async model => {
      ...
      try {
        ...
        await services.streamQuery(created.streamUrl, event => {
          ...
          if (event.type === 'answer.delta') { text += event.payload.text ?? ''; update(model.id, { text }, run); }
          if (event.type === 'answer.completed') update(model.id, {
            text: event.payload.answer ?? text, evidence: event.payload.evidence ?? [], labels: event.payload.labels,
