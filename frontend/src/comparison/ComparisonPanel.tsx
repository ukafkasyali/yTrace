...
export default function ComparisonPanel({ data, interval, cases, services, onEvidence }: Props) {
  ...
  return <section className="comparison-panel" aria-label="Incident comparison">
    ...
    <section className="incident-cohort" aria-labelledby="incident-cohort-title">
      ...
      {!matching && matches.length > 0 && <div className="incident-matches">{matches.map((match, index) => <button key={match.caseId} className="incident-match" onClick={() => void chooseMatch(match)}><span className="incident-rank">{index + 1}</span><span><strong>{match.title}</strong><small>{match.recordingId} · {intervalLabel(match.interval)} · strongest range {match.strongestJoint?.replace('joint_', 'J') ?? 'n/a'}</small></span><span className="incident-score">{match.score.toFixed(2)}<small>similarity</small></span></button>)}</div>}
