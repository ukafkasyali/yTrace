...
type Props = { data: DemoData; datasetId: string; playhead: number; interval: Interval; services: Services; registry: ModelRegistry; onEvidence: (e: EvidenceLink) => void; onCompare: (question?: string) => void };
export default function AssistantPanel({ data, datasetId, playhead, interval, services, registry, onEvidence, onCompare }: Props) {
  ...
  const connectionNote = registry.loading ? 'Checking model status…' : registry.error ? 'Model status unverified · local calculations available' : assistantAvailable ? 'LLM assistant available · choose an analysis mode' : directAvailable.length ? `${directAvailable.map(model => model.label).join(', ')} available in Compare models · local calculations here` : 'Local calculations available · LLM assistant unavailable';
  ...
  return <section className="assistant-panel" aria-label="Telemetry assistant">
    <header className="panel-heading"><div className="assistant-title"><MessageSquare size={16}/><h2>Ask the signal</h2></div><button className="text-button" onClick={() => onCompare(question.trim() || messages.at(-1)?.question || undefined)}>Compare models <ArrowUpRight size={14}/></button></header>
