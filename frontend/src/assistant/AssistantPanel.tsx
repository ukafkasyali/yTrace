...
export default function AssistantPanel({ data, datasetId, playhead, interval, services, onEvidence, onCompare }: Props) {
  ...
  useEffect(() => { if (!messages.length) return; body.current?.scrollTo({ top: body.current.scrollHeight, behavior: 'instant' }); }, [messages]);
  ...
  return <section className="assistant-panel" aria-label="Telemetry assistant">
    ...
    <div className="conversation" ref={body} aria-live="polite">
      {!messages.length && <div className="conversation-intro"><div className="assistant-emblem"><Waves size={22}/></div><h3>Start with what changed.</h3><p>Ask about the selected interval. Follow the evidence back to any joint.</p><div className="suggestion-list">{['Which joints have the largest torque range?', 'Where is the strongest absolute peak?', 'How does variability change in this interval?'].map(q => <button key={q} onClick={() => void submit(q)}><span>{q}</span><ArrowUpRight size={14}/></button>)}</div><div className="connection-note"><Terminal size={13}/><span>Numerical tools are available. Models are not simulated.</span></div></div>}
