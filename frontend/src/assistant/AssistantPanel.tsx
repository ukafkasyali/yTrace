...
export default function AssistantPanel({ data, datasetId, playhead, interval, services, registry, onEvidence, onModelWindow }: Props) {
  ...
  return <section className="assistant-panel" aria-label="Telemetry assistant">
    ...
    <div className="model-input-status"><div><strong className="mono">{intervalLabel(interval)}</strong><span>{windowIssue ? 'Selection needs attention' : '1.024 s · 7 joints · raw telemetry'}</span></div><button className="btn btn-primary" disabled={busy || !assistantAvailable || Boolean(windowIssue)} onClick={() => { setMode('assistant'); void submit('Analyze this robot telemetry window.'); }}>{busy ? 'Analyzing…' : 'Analyze interval'}</button></div>
