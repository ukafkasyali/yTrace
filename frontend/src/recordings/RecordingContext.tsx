...
type Props = { datasetId: string; data: DemoData; playhead: number; interval: Interval; highlighted: string[]; onMarker: (marker: Marker) => void; onHighlight: (id: string) => void; onData: () => void };
export default function RecordingContext(props: Props) {
  const [view, setView] = useState<'overview' | 'robot'>('overview');
  const isKuka = props.datasetId === 'zenodo-21927431';
  return <section className="recording-context" aria-label="Recording context">
    <div className="context-switch" role="group" aria-label="Recording context view">
