...
export default function RecordingExplorer({ data, playhead, interval, highlighted, onMarker, onHighlight, onData }: Props) {
  ...
  return <section className="explorer-panel" aria-label="Recording explorer">
    ...
    <div className="recording-title"><h1>{data.recording.archive ? "KUKA LWR4+" : data.recording.name}</h1><p>Recorded torque experiment <span>{data.channels.length} joints · {data.recording.durationSeconds.toFixed(0)} s · {data.recording.sampleRateHz} Hz source</span></p></div>
    ...
    <footer className="explorer-footer"><Info size={13}/><span>Publisher markers are annotations, not verified collision onset.</span><span className="data-resolution">{stats?.resolution === 'raw' ? 'Raw samples' : 'Display samples'}</span></footer>
  </section>;
}
