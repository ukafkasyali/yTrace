...
export default function RecordingExplorer({ data, playhead, interval, highlighted, onMarker, onHighlight, onData }: Props) {
  ...
  const selectedMarkerWindow = selectedMarker ? markerWindow(selectedMarker, data.recording.durationSeconds) : null;
  const selectedMarkerHasRaw = selectedMarkerWindow ? selectedMarkerWindow.start >= data.detail.startSeconds && selectedMarkerWindow.end <= data.detail.endSeconds : false;
  const stats = useMemo(() => {
  ...
  return <section className="explorer-panel" aria-label="Recording explorer">
    ...
    <div className="explorer-body">
      <aside className="event-browser"><div className="section-label"><Flag size={12}/>All markers <span>{data.events.length}</span></div><div className="event-list" aria-label="All publisher markers">{data.events.map(e => <button key={e.id} onClick={() => onMarker(e)} className={`event-item ${e.timeSeconds >= interval.start && e.timeSeconds < interval.end ? 'selected' : ''}`}><span className="event-tick"/><span>{e.label}<small>{e.timeSeconds.toFixed(3)} s</small></span><ChevronRight size={13}/></button>)}{!data.events.length && <p className="quiet-empty">No publisher markers in this recording.</p>}</div>{next && <button className="text-button next-marker" onClick={() => onMarker(next)}>Next marker <ChevronRight size={13}/></button>}</aside>
      <div className="range-overview"><div className="range-title"><h3>Torque range</h3><span className="mono">{intervalLabel(interval)}</span></div><div className="range-bars" aria-label="Maximum minus minimum torque per joint">{data.channels.map((channel, i) => { const value = stats?.ranges[i]?.range; return <button key={channel.id} className={`range-row ${highlighted.includes(channel.id) ? 'active' : ''}`} onClick={() => onHighlight(channel.id)} aria-label={`Highlight ${channel.name}, range ${value?.toFixed(3) ?? 'unavailable'} Nm`}><span>J{i+1}</span><div className="bar-track"><div style={{ width: `${(value ?? 0)/maximum*100}%`, background: COLORS[i] }}/></div><span className="mono">{value?.toFixed(3) ?? 'n/a'} <small>Nm</small></span></button>; })}</div><div className="range-caption">{leading ? <><span>{leading.name}</span> has the widest excursion in this interval.</> : 'Select a replayed interval to compare joints.'}</div></div>
    </div>
    <footer className="explorer-footer"><details className="marker-explainer"><summary><Info size={13}/><span>{selectedMarker ? `${selectedMarker.label} · marker details` : 'What is a marker?'}</span></summary><div className="marker-detail"><strong>{selectedMarker ? `${selectedMarker.label} at ${selectedMarker.timeSeconds.toFixed(3)} s` : 'Publisher event marker'}</strong><p>This timestamp comes from <span className="mono">{selectedMarker?.source ?? 'JK_moments'}</span>, the dataset publisher's manual event annotation. It helps navigate the recording, but it is not an independently verified exact collision onset.</p>{selectedMarker && <p className="marker-analysis-status">{selectedMarkerHasRaw ? 'Selecting it opens the surrounding raw window. Run Analyze interval to start measurements and OpenTSLM.' : 'Selecting it opens the available display samples. OpenTSLM requires a full-resolution window from the data adapter.'}</p>}{selectedMarker && <button className="text-button" onClick={() => onMarker(selectedMarker)}>Open marker context <ChevronRight size={13}/></button>}</div></details><span className="data-resolution">{stats?.resolution === 'raw' ? 'Raw samples' : 'Display samples'}</span></footer>
  </section>;
}
